...
项目提供统一的维护命令 `maintenance.py`，把五项日常检查整合到一个入口，改数据或新增示例后只需跑一次：
...
# 或单独跑某一项
python maintenance.py profile    # 刷新数据 profile 缓存 (.metadata_cache/)
python maintenance.py readme     # 更新 README 自动生成区块 (output/readme/)
python maintenance.py manifest   # 校验 examples_manifest.json 与磁盘一致性
python maintenance.py deps       # 检查依赖锁文件 (requirements.txt vs .piptools)
python maintenance.py launcher   # 启动器栈自检 (schema + manifest + launcher.py)
```
...
| 子命令 | 检查什么 | 什么时候跑 |
|--------|----------|------------|
| `profile` | 扫描 `code/data/raw/` 下的 CSV / Excel，重建 `.metadata_cache/*.json`（行数、列数、缺失值） | 新增或更新数据文件后 |
| `readme` | 扫描 `code/` 下的 notebook 和 `output/` 下的产物，重写 `output/readme/` 下两个自动生成区块 | 新增 notebook 或输出新产物后 |
| `manifest` | 逐条校验 `examples_manifest.json`：字段完整、type 在 schema 中、脚本/数据文件存在、无未声明脚本 | 新增示例或改文件名后 |
| `deps` | 对比 `requirements.piptools` 与 `requirements.txt`，跑 `pip check` 检查安装依赖一致性 | 改依赖或换环境后 |
| `launcher` | 校验 `launch_schema.json` 字段完整、`manifest` 中 type 使用情况、`launcher.py` 语法 | 改启动规则或新增 type 后 |

所有子命令的 PASS / WARN / FAIL 会分组打印，最终有任一 FAIL 时退出码为 1。只改了某一类内容时，跑对应的子命令即可；不确定时跑 `python maintenance.py`（等价于 `all`）。
